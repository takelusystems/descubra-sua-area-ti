...
const areaVisuals: Record<TechAreaIcon, AreaVisual> = {
  hardware: {
    ...
    backgroundClass: 'from-cyan-400/10 to-blue-500/[0.03]',
  },
  ...
  programming: {
    ...
    backgroundClass: 'from-violet-400/10 to-purple-500/[0.03]',
  },
  ...
  networks: {
    ...
    backgroundClass: 'from-blue-400/10 to-cyan-500/[0.03]',
  },
  ...
  cybersecurity: {
    ...
    backgroundClass: 'from-emerald-400/10 to-green-500/[0.03]',
  },
  ...
  games: {
    ...
    backgroundClass: 'from-fuchsia-400/10 to-pink-500/[0.03]',
  },
  ...
  ai: {
    ...
    backgroundClass: 'from-amber-400/10 to-orange-500/[0.03]',
  },
}
...
function ResultPage({
  ...
  const mainProfile = resultProfiles[mainScore.area]
  const mainVisual = areaVisuals[mainScore.area]

  const MainIcon = mainVisual.icon

  const difference =
    mainScore.percentage - secondScore.percentage
        ...
        <div className="mx-auto max-w-6xl">
          <section className="text-center">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-slate-500 sm:text-base">
              Analisamos suas respostas e comparamos sua
              compatibilidade com as seis áreas avaliadas.
            </p>
          </section>
              ...
              <div className="flex lg:justify-end">
                <div className="min-w-32 rounded-2xl border border-white/[0.08] bg-black/20 px-6 py-5 text-center backdrop-blur-xl">
                  <p className="text-4xl font-black tracking-tight text-white sm:text-5xl">
                    {Math.round(mainScore.percentage)}
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-400">
                    {mainArea.name} e {secondArea.name} ficaram
                    separados por apenas{' '}
                    <strong className="text-slate-200">
                      {difference.toFixed(1).replace('.', ',')}
                      {' '}pontos percentuais
                    </strong>
                    . Isso indica que você pode ter afinidade
                    relevante com os dois caminhos, e eles podem
                    inclusive se complementar.
                  </p>
                </div>
              </div>
            </section>
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
            <section className="rounded-[2rem] border border-white/[0.07] bg-[#090c13]/80 p-6 sm:p-8">
              ...
              <p className="mt-5 text-sm leading-7 text-slate-500">
                Além de {mainArea.name}, seu resultado também
                apresentou afinidade com{' '}
                <strong className="text-slate-300">
                  {getAreaById(secondaryScores[0].area).name}
                </strong>{' '}
                e{' '}
                <strong className="text-slate-300">
                  {getAreaById(secondaryScores[1].area).name}
                </strong>
                , o que mostra que seus interesses podem combinar
                conhecimentos de diferentes áreas da tecnologia.
              </p>
            </section>
          ...
          <section className="mt-8 rounded-[2rem] border border-white/[0.07] bg-[#090c13]/80 p-6 sm:p-8">
            ...
            <div className="mt-8 space-y-4">
              {result.ranking.map((score, index) => (
                <CompatibilityRow
                  key={score.area}
                  score={score}
                  position={index + 1}
            ...
            <div className="mt-7 rounded-2xl border border-white/[0.05] bg-white/[0.02] p-4">
              <p className="text-xs leading-6 text-slate-600">
                Os percentuais representam sua compatibilidade
                individual com cada área. Por isso, eles não
                precisam somar 100%.
              </p>
            </div>
          </section>
          ...
          <section className="mt-8 rounded-[2rem] border border-blue-400/10 bg-gradient-to-br from-blue-400/[0.05] to-violet-400/[0.03] p-6 sm:p-8">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-400">
              Este quiz possui finalidade educativa e orientativa.
              O resultado ajuda a explorar possibilidades dentro da
              tecnologia, mas não determina sua profissão nem limita
              as áreas que você pode aprender. Interesses e habilidades
              também podem mudar conforme você conhece novas
              experiências.
            </p>
          </section>
